    ...
    <q-page padding>
        <div class="myCard">
            ...
            <client-only>
                ...
                <vue-cal :class="{ 'vuecal--dark': isDarkActive }" :disable-views="['year', 'years']"
                    default-view="week" :timeStep="60" :eventsOnMonthView="false" small
                    @cell-dblclick="cellDblClick($event)" :events="events" :time-from="8 * 60" :time-to="20 * 60"
                    :locale="getLanguage" today-button
                    :today-button-style="{ color: 'white', border: '1px solid white' }"
                    :editable-events="{ title: false, drag: true, resize: false }" :drag-to-create-event="false"
                    @event-drag-create="onEventDragCreate" @event-drop="onEventDrop" :disable-drag="false"
                    :drag-to-create-threshold="10" :drag-to-move-threshold="10" :snap-to-time="60"
                    :drag-to-move-events="{ enable: true }" :event-editable="false" @event-title-change="onTitleChange"
                    @event-resize="onEventResize" @event-duration-change="onDurationChange"
                    @event-drag-start="onDragStart" @event-drag-end="onDragEnd">
                    ...
                    <template #event="{ event }">
                        ...
                        <q-badge rounded floating class="edit-badge" @click="onEditEvent(event)"
                        ...
                        <q-tooltip>
                            <div style="display: grid">
                                ...
                                <span class="vuecal__event-time" style="overflow: hidden; text-align: center">
                                    {{ formatDate(event.start, "YYYY-MM-DD HH:mm", "HH:mm") }} -
                                    {{ formatDate(event.end, "YYYY-MM-DD HH:mm", "HH:mm") }}
                                </span>
                                <span style="overflow: hidden; text-align: center">
